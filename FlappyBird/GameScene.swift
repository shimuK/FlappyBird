...
class GameScene: SKScene, SKPhysicsContactDelegate {
    ...
    let player = try! AVAudioPlayer(data: NSDataAsset(name: "sound")!.data)
    ...
    func setupApple() {
        ...
        // Item位置の上下の振れ幅を鳥のサイズの3倍とする
        let random_y_range = birdSize.height * 3
        
        // ItemのY軸下限位置(中央位置から下方向の最大振れ幅でAppleを表示する位置)を計算
        let groundSize = SKTexture(imageNamed: "ground").size()
        let center_y = groundSize.height + (self.frame.size.height - groundSize.height) / 2
        let apple_lowest_y = center_y - appleTexture.size().height / 2 - random_y_range / 2
        ...
        let createAppleAnimation = SKAction.run({
            ...
            let apple_y = apple_lowest_y + random_y

            // Itemを作成
            let sprite = SKSpriteNode(texture: appleTexture)
            sprite.position = CGPoint(x: sprite.size.width * 3, y: apple_y)
            
            // 物理演算を設定
            sprite.physicsBody = SKPhysicsBody(circleOfRadius: sprite.size.height / 2)
            
            // 衝突のカテゴリー設定
            sprite.physicsBody?.categoryBitMask = self.appleCategory
            
            // 衝突の時に動かないように設定する
            sprite.physicsBody?.isDynamic = false
            apple.addChild(sprite)
